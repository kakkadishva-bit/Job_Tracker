...
if _frontend_url:
    ...
    CORS(app, origins="*")  # dev convenience — override with FRONTEND_URL in prod
...
def get_related_roles(role: str) -> List[str]:
    """Related titles only from curated data — never fabricated.

    - canonical/alias query → its ROLE_MAP list (each related title keeps
      its curated name, never the query relabeled);
    - unknown query → [query] alone, so /api/search renders exactly one
...
# ═══════════════════════════════════════════════════════════════════════
#  SKILLS DATABASE (for matching & gap analysis)
# ═══════════════════════════════════════════════════════════════════════
...
ROLE_GUIDES_DB: Dict[str, Dict[str, Any]] = {
    "frontend developer": {
        ...
        "career_growth": [
            "Junior Frontend Developer → Mid-Level Frontend Developer",
            "Mid-Level → Senior Frontend Developer",
            "Senior → Frontend Architect or Engineering Manager",
    ...
    "python developer": {
        ...
        "career_growth": [
            "Junior Python Developer → Mid-Level Developer",
        ...
        "growth_outlook": "Excellent — Python dominates in AI/ML, data science, web development, and automation. Demand continues to grow across all sectors."
    },
    "data scientist": {
        ...
        "career_growth": [
            ...
            "Senior → Principal Data Scientist / ML Engineering Manager",
            "Staff → VP of Data / Chief Data Officer",
        ...
        "industry_demand": "Very High",
        "growth_outlook": "Exceptional — Every industry needs data-driven decision making. AI/ML specialization commands premium salaries."
    },
    "full stack developer": {
        ...
        "career_growth": [
            "Junior Full Stack Developer → Mid-Level Developer",
            "Mid-Level → Senior Full Stack Developer",
            "Senior → Tech Lead / Architect",
            "Staff → Principal Engineer / VP of Engineering",
        ...
        "industry_demand": "Very High",
        "growth_outlook": "Excellent — Full-stack skills are highly valued, especially with modern frameworks. Companies seek versatile developers who can own features end-to-end."
    },
    "devops engineer": {
        ...
        "career_growth": [
            "Junior DevOps Engineer → Mid-Level DevOps Engineer",
            "Mid-Level → Senior DevOps / SRE",
            "Senior → Platform Engineering Manager",
            "Staff → VP of Infrastructure / CTO",
        ...
        "industry_demand": "Very High",
        "growth_outlook": "Excellent — Cloud-native, Kubernetes, and platform engineering are top priorities. Every company with digital products needs DevOps expertise."
    },
    "machine learning engineer": {
        ...
        "career_growth": [
            "Junior ML Engineer → Mid-Level ML Engineer",
            "Mid-Level → Senior ML Engineer",
            "Senior → Staff ML Engineer / ML Architect",
            "Staff → Director of ML / VP of AI",
        ...
        "industry_demand": "Very High",
        "growth_outlook": "Exceptional — AI/ML is the most in-demand field in tech. Generative AI expertise commands premium compensation."
    },
    "data analyst": {
        ...
        "career_growth": [
            ...
            "Senior → Analytics Manager / Data Science Manager",
        ...
        "industry_demand": "High",
        "growth_outlook": "Strong — Every organization needs analysts to drive data-informed decisions. Entry point into data careers."
    },
    "ui/ux designer": {
        ...
        "career_growth": [
            "Junior UI/UX Designer → UX/UI Designer",
            "Designer → Senior UX Designer",
            "Senior → Design Lead / Design Manager",
            "Staff → Head of Design / VP of Design",
            "Specialization paths: UX Research, Interaction Design, Design Systems, Motion Design, Product Design"
        ],
        "interview_topics": [
            "Design process: research → wireframe → prototype → test",
        ...
        "industry_demand": "High",
        "growth_outlook": "Strong — User experience is a key differentiator. Companies invest heavily in design to retain users and improve conversion."
    },
    "ai engineer": {
        ...
        "career_growth": [
            ...
            "Senior → Staff AI Engineer / AI Architect",
            "Staff → Director of AI / VP of AI",
        ...
        "industry_demand": "Very High",
        "growth_outlook": "Exceptional — The AI revolution is creating unprecedented demand. Every company is exploring AI integration."
    },
}


# Aliases users commonly type → canonical guide key. Displayed titles
...
def get_role_guide(role: str) -> Dict[str, Any]:
    """Return the DISTINCT guide for a role — no shared generic copy.

    Rules:
    - exact match → that role's own curated guide verbatim;
    - known alias / unambiguous containment → that guide, but the
      displayed title keeps the canonical name so two different queries
      never show identical bodies under different titles;
    - anything else → honest "no dedicated guide yet" payload listing the
    ...
    # No curated guide exists — try to generate one via LLM.
    ...
    # Hard fallback: LLM also unavailable → honest generic guide with real
...
def get_role_skills(role: str) -> List[str]:
    ...
    Resolution mirrors get_role_guide (exact → alias → containment) and
...
def analyze_resume_text(text: str, target_role: str = "") -> Dict[str, Any]:
    ...
    if not has_bullet_points:
        suggestions.append({
            ...
            "improvement": "Use bullet points (•) to list achievements and responsibilities. ATS systems and recruiters prefer scannable formats.",
            "priority": "High",
            "impact": "+5% ATS score"
        })
...
CAREER_INSIGHTS_DB = {
    "python developer": {
        ...
        "avg_salary_inr": "₹8,00,000 - ₹25,00,000",
    ...
    "frontend developer": {
        ...
        "avg_salary_inr": "₹6,00,000 - ₹20,00,000",
        "demand_level": "High",
    ...
    "full stack developer": {
        ...
        "avg_salary_inr": "₹7,00,000 - ₹22,00,000",
        "demand_level": "Very High",
    ...
    "devops engineer": {
        ...
        "avg_salary_inr": "₹10,00,000 - ₹30,00,000",
        "demand_level": "Very High",
    ...
    "machine learning engineer": {
        ...
        "avg_salary_inr": "₹12,00,000 - ₹40,00,000",
        "demand_level": "Very High",
...
def get_career_insights(role: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "avg_salary_inr": "₹5,00,000 - ₹18,00,000",
...
def api_search():
    ...
    # The primary query and each related title resolve to their OWN guide
    # (or an honest "no guide" card) — dedupe by title so the grid never
    # shows the same body twice under different names.
    ...
    # Localize salary figures: profile location wins if set, IP is only
    # a fallback guess for logged-out/no-profile-location users, never
    # an override of what the user explicitly set.
...
def api_expand_roles():
    ...
    return jsonify({"role": role, "related_roles": related})
...
def api_list_applications():
    ...
    applications = JobApplication.query.filter_by(user_id=current_user.id) \
        .order_by(JobApplication.last_updated.desc()).all()
...
def api_list_notifications():
    """Generates any new notifications (interview reminders, new job
    matches for saved preferred roles) then returns the unread ones.
    There's no background scheduler yet, so this check runs lazily each
    time the user opens the app - see services/notifications/engine.py
    for how to move this onto a real schedule."""
...
def api_interview_prep_answer():
    ...
    #    skill BEFORE the topic changes; strong answers get a deeper question.
...
def api_get_feedback():
    ...
    try:
        ...
        # Get positive reviews first (rating >= 4), then by date
...
def api_chat_accept_policy():
    """Accept the chat privacy policy."""
    status = _get_chat_status(current_user)
    if status.is_banned:
    ...
    status.accepted_at = datetime.utcnow()
    db.session.commit()
    
    # System message announcing the user joined
    welcome = ChatMessage(
        user_id=current_user.id,
    ...
    return jsonify({'success': True, 'message': 'Policy accepted. Welcome to the community chat!'})
...
def api_chat_users():
    ...
    # Get all users who have accepted the policy
